...
| File | Role |
|------|------|
| `menu.py` | **`Menu`** — draws a scrollable option list on the 128×64 OLED. **`SDRApp`** — wires I2C/OLED, encoder, and button; owns the frequency list and `NavStack`; main event loop at 50 ms. |
...
1. **Add Frequency** — tune `current_freq_input` in `step_size` (1 kHz) increments, save to list.
2. **Remove Freq** — delete a stored frequency.
3. **Listen Freq** — select a frequency (currently prints to console; Si5351 retune not yet wired in UI).
...
1. `initialize()` — mount SD, create `DAC` in blocking stereo mode (muted).
2. `play(filename)` — skip WAV header to PCM `data` chunk, start asyncio task.
3. `_stream_task` — reads 16 KB chunks from the file, writes to I2S via `asyncio.StreamWriter.drain()` (yields while the ring fills). Unmutes after two buffered chunks; on EOF waits 300 ms for the ring to drain before muting.
...
| Feature | Detail |
|---------|--------|
| Pins | BCK=15, WS=16, DATA=14, XSMT mute=6 |
| Modes | Stereo (default) or mono via `set_stereo()` / `set_mono()` |
| Blocking vs non-blocking | `nonblocking=True` registers an IRQ drain handler; `PlaybackController` uses blocking mode with asyncio `StreamWriter` instead |
...
- **Audio/UI not integrated** — `PlaybackController` and `SDRApp` are independent; playing WAVs while navigating the menu would require merging the asyncio loop with the menu's blocking `while True` loop (or moving the menu to asyncio).
